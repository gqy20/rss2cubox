...
_TABLE: list[tuple[str, str, Any, str, str]] = [
    ...
    ("SIGNAL_CLUSTER_MAX_TURNS", "int", "12", "prediction", "cluster agent 的轮数上限。原先是 200，daily_report 同样 200 导致实跑 25 轮、烧掉 798K input tokens"),
    ("SIGNAL_CLUSTER_MAX_DETAIL_READS", "int", "5", "prediction", "prompt 里允许 agent 按 ref 精确 Grep 明细文件的次数上限。索引已覆盖全部文章，翻文件只应用于边界判断"),
    ...
    ("SIGNAL_CLUSTER_AGENT_TIMEOUT_SECONDS", "float", "900", "prediction", "cluster 单次调用超时。原默认 300 实测必然 TimeoutError → clusters=0 → 整条闭环空转"),
